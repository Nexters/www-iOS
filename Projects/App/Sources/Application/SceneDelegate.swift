    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
            ...
            if UserDefaultKeyCase().getOnBoarding() {
                ...
                self.window?.rootViewController = UINavigationController(rootViewController: PlaceVoteViewController(viewmodel: PlaceVoteViewModel(usecase: PlaceVoteUseCase(repository: PlaceVoteDAO(network: VoteAPIManager.provider)), roomId: 73, status: .voting)))
            } else {
